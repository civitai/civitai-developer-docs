...
const meshyV7MultiImageBody = {
  steps: [{
    ...
    input: {
      ...
      imageUrls: [sampleImage, sampleImage],
      topology: 'quad',
      targetPolycount: 50000,
    },
  }],
...
<RecipeRun :body="meshyTextBody" :wait="0" />
...
<RecipeRun :body="meshyImageBody" :wait="0" />
...
<RecipeRun :body="meshyRiggedBody" :wait="0" />
...
<RecipeRun :body="meshyV7ImageBody" :wait="0" />

Unlike v6, v7 prices per feature — a bare mesh is cheaper than a textured one, and `ultraMode`, rigging and animation each add to the bill. See [Cost](#cost).
...
<RecipeRun :body="meshyV7MultiImageBody" :wait="0" />
...
| `animationActionId` | | `92` | Meshy animation preset, `0`–`696` (`0` is "Idle"). Only used with `enableAnimation`. See Meshy's [animation library](https://docs.meshy.ai/en/api/animation-library). |
...
<RecipeRun :body="tripoImageBody" :wait="0" />
...
<RecipeRun :body="tripoQuadBody" :wait="0" />
...
<RecipeRun :body="hunyuanImageBody" :wait="0" />

`v2-mini` is the quickest way to preview a mesh:

<RecipeRun :body="hunyuanMiniBody" :wait="0" />
