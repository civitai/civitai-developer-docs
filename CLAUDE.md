...
- 🔴 **A sidebar array registered under TWO route keys is emitted TWICE in `llms.txt`.** VitePress ROUTES a sidebar, and a leaf page keeps one by ALIASING an existing array under a second key (`'/apps/examples': appsGuideSidebar`). `vitepress-plugin-llms` routes nothing — it does `Object.values(sidebar).flat()`. That shipped: `### Guide`, `### Examples` and `### Design system` each appeared twice on prod, 23 duplicated lines, while the rendered HTML, the nav and every on-page sidebar stayed correct — so **`llms.txt` is the only surface that can show this, and it is the one nobody reads.** The plugin now takes `dedupeSidebarGroups(sidebar)`, not `sidebar`; `check:built-site` asserts it on the BUILT artifact, because the bug is invisible in the source object.
- `npm run check:agent-setup` grades five things, all offline: (1) every `civitai …` command and flag named in `public/agent-setup/prompt.md` exists in the CLI help snapshot, AND every non-`civitai` binary that file tells an agent to run is on the `ALLOWED_NON_CIVITAI` allowlist in the script, in an allowed subcommand/argument shape (it is fetched and executed unattended, so a new third-party command must be an explicit one-line diff a reviewer sees); (2) the landing page's copy-paste string matches `.vitepress/agent-setup.mjs`, every absolute URL the page carries outside the generated region is `PROMPT_URL` or on `ALLOWED_PAGE_URLS`, and every prompt-claiming URL in `prompt.md` is `PROMPT_URL`; (3) `nginx.conf` serves `PROMPT_PATH` from an exact-match `location =`; (4) the inline copy of the prompt on `agent-setup/index.md` is byte-for-byte what `npm run gen:agent-setup-page` would write; (5) `public/agent-setup/prompt.md` sits inside a byte budget, with a ceiling bounding the budget and a floor as the positive control. 🔴 **Check 5 is a RATCHET AGAINST GROWTH, not a guarantee an agent receives the whole file** — the measured loss (an LLM summary that dropped the install-failure section, both MCP URLs and all of step 5) happened at 6,949 bytes, INSIDE the budget it permits; lossy summarisation is a property of the consumer and nothing here can assert it away. What it buys is that the file cannot go 2,798 → 6,232 → 6,949 bytes in three commits again with nothing measuring the total. The three constants are derived from the file's own section sizes — headroom smaller than its smallest section, so no new step fits without a reviewable raise — and the floor is derived from history (2,798 B was the smallest COMPLETE version ever shipped), deliberately weak so a future deliberate trim is welcomed rather than fought. The sequencing is SETTLED and this line used to say the opposite: check 1 was held red until civitai/cli shipped `civitai agent-setup` (civitai/cli#528) and this repo re-captured the snapshot, and both have happened — it is green now. The script's own `SEQUENCING NOTE` is the authority; see `.github/workflows/agent-setup.yml` for the job.
...
- `npm run check:ai-catalog` guards machine discovery — `public/.well-known/ai-catalog.json` plus the `Link:` header in `nginx.conf` — and **every assertion it makes is about the RELATIONSHIP between those two files**, because each is individually plausible while being collectively wrong: a catalog listing artifacts nobody is told about, or a header promising URLs the catalog never heard of. Both review clean alone. Offline (gates a PR): the catalog parses and carries its shape; at least `MIN_CATALOG_ENTRIES` entries, which is the built-in positive control; every entry URL absolute-https on `developer.civitai.com`; `set $ai_link` defined exactly once; the catalog is advertised, and every *other* advertised URL is itself an entry. Scheduled, never a gate: every catalogued and advertised URL resolves 200 with the Content-Type the catalog declares, and the LIVE origin sends a `Link` equal to the config's — the only check that can see a config which was never deployed. It must stay importable with no `node_modules` (its CI job runs without `npm ci`).
  - 🔴 **The network half CANNOT gate a PR, and the reason is structural.** The origin serves what is on `main`, so a PR that ADDS a catalogued URL is red until it merges and deploys, by construction — gating on it would make every such PR unmergeable. A 404 is therefore never treated as an outage (it is the exact symptom the guard reports); only DNS/timeout/5xx skip, loudly, with a summary that always prints how many URLs were actually checked.
  - 🔴 **The ledger is "every `location` that sets ANY header must also set `Link`", NOT a list of routes** — nginx drops every inherited `add_header` from a block declaring one of its own, so "sets a header" is exactly the predicate deciding whether a location needs its own copy. A location in neither the advertise set nor `LINK_EXEMPT_LOCATIONS` fails the run, and an exemption naming a location that no longer exists fails too, so the set cannot drift in either direction. Measured under a real nginx with one server-level copy and no per-location ones: **four of five document routes served no `Link`** — `/apps/guide.html`, `/agent-setup/prompt.md`, `/.well-known/ai-catalog.json` and **`/` itself** (resolved through an internal redirect into `location ~* \.html$`) — while `/llms.txt` kept it, which is what proves the server-level directive was working at all. Spot-checking the homepage would have passed that config.
  - 🔴 **`default_type` does NOT set the catalog's Content-Type; the empty `types { }` block does.** `default_type` is the fallback for an extension nginx cannot resolve — it works for `.md` precisely because `.md` is absent from `mime.types`. `.json` is present, so the map wins and the response comes back `application/json`. Measured on a real nginx against this exact file. The empty block looks like a no-op, and deleting it silently reverts the declared type.
  - **Deliberately NOT published:** `.well-known/agent-skills/index.json` — every entry in that format is a `tar.gz`/`SKILL.md` with a `sha256:` digest, and this project ships no skill bundles; `llms.txt` is already the flat index, and a flat index measured 0.462 against 0.267 for a hierarchy. An index with zero or invented entries reads as coverage while providing none. The two MCP endpoints are out for a different reason: a `GET` to `mcp.civitai.com/mcp` is 405 and `orchestration.civitai.com/mcp` is 401 — transports, not fetchable documents. The entry that would represent them is a `.well-known/mcp/server-card.json`, which this site does not serve yet.
- `npm run check:example-apps` guards `apps/examples.md` — the ONE URL the `civitai agent-setup` managed block points an agent at instead of embedding a repo list in every project. Offline (gates a PR): its own fixture tables, the page parsing and still naming ≥ `MIN_EXAMPLE_REPOS` repositories, the counts and `#anchor`s the page states about itself, every example section stating a gradeable scope list, and the hook-list date stamp. Scheduled, never a gate: each repository's `full_name` read back from the API (**a 200 is not evidence — GitHub 301-redirects a rename and `fetch` follows it silently**), and each example's scope list compared against that repository's own `block.manifest.json`.
  - 🔴 **Scopes are a checked claim; the HOOK lists are only DATED.** Verifying what eight repositories import needs source parsing the guard deliberately does not do, so the page carries `Hook lists verified by hand on <date>` and the guard asserts that stamp is present, a real date and not in the future, printing its age on every run. It does **not** fail on age — a stamp that reddens after N days is a permanently-red gate with a countdown. Do not "fix" it into one.
  - 🔴 **The scope comparison is by SET, not sequence, on purpose.** The page's claim is about *which* scopes an example declares, so a manifest that merely reorders its array leaves the sentence true and failing on it would be a false-fail; order drift is a non-fatal note instead. Manifests are fetched from **`raw.githubusercontent.com`, not `api.github.com`** — that keeps the sweep at exactly one api.github.com GET per repository, so the scope check did not move it closer to the unauthenticated 60/hr shared-runner limit. Unreachable → loud SKIP, exit 0; a 404, unparseable JSON, or no `scopes` array → FAIL, with the cause named.
- 🔴 **`appblocks-drift.yml` has a second job that DELIVERS its red, and it is this repo's SECOND `permissions:` elevation** (the first is `cli-snapshot-refresh.yml`'s `refresh` job — a comment there claimed to be the only one and was corrected). Every check on that schedule detected upstream movement and surfaced it nowhere: a red run on a scheduled workflow is indistinguishable from nobody looking. `scripts/drift-notify.mjs` (`npm run notify:drift`) opens or refreshes ONE GitHub issue and closes it on the next clean run, from a `notify` job holding `issues: write` and running no `npm ci` — the privileged job executes no third-party code. **It alerts on "nothing was verified" too** — a job that was skipped or cancelled, a missing report, zero repositories verified, or zero scope lists verified — because a silent zero and a clean bill of health print the same thing and carry the same exit code. Its decision table self-tests on every invocation and is PR-gated by a `--self-test` step in `example-apps.yml`; both scripts must stay importable with **no `node_modules`**.
